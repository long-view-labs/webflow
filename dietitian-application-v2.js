(async () => {
  ...
  async function initDraft() {
    ...
    if (!data || !data.applicationPublicId || !data.flowRuntime) {
  ...
  function ensureApplicationStyles() {
    if (document.getElementById("nourish-application-v2-styles")) return;
    const style = document.createElement("style");
    style.id = "nourish-application-v2-styles";
    style.textContent = `
      #gh-title{
  ...
  async function syncLeadStatus(form, status) {
    ...
    if (runtimeContext.applicationPublicId) {
      requestBody.applicationPublicId = runtimeContext.applicationPublicId;
    }
  ...
  function buildForm(schema) {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (runtimeContext.applicationPublicId) {
          fd.append("applicationPublicId", runtimeContext.applicationPublicId);
        }
        ...
        if (resp.ok) {
          ...
          if (
            ...
            try {
              ...
              window.rudderanalytics.track("provider_application_submitted", {
                email: submittedEmail || undefined,
                phone: e164Phone,
                applicationPublicId:
                  runtimeContext.applicationPublicId || undefined,
                flowRuntime: FLOW_RUNTIME.LEGACY,
              });
  ...
  function trackApplicationStarted() {
    if (!applicationStartedTracked) {
      ...
      trackEvent("provider_application_started", {
        applicationPublicId: runtimeContext.applicationPublicId || undefined,
        flowRuntime: FLOW_RUNTIME.LEGACY,
      });
  ...
  if (
    ...
    runtimeContext.iframeUrl
  ) {
    mountSteppedIframe(runtimeContext.iframeUrl);
  } else if (schema) {
    buildForm(schema);
  } else {
    console.error("Cannot render legacy form: schema missing");
  }
})();
